...
export function renderSidebar(container, {
  ...
  if (activeUser && userBtn) {
    ...
    if (onUserSwitch) userBtn.addEventListener('click', onUserSwitch);
  }
  ...
  for (const conv of conversations) {
    ...
    const displayName = escapeHtml(
      conv.contact || conv.participants?.[0] || conv.id
    );
  ...
  el.showCalls = (panel) => {
    ...
    const tab = panel.classList.contains('payments-panel')
      ? 'payments'
      : panel.classList.contains('media-panel')
        ? 'media'
        : 'calls';
    el.classList.toggle('sidebar--payments', tab === 'payments');
    for (const btn of bottomNav.querySelectorAll('.sidebar-bottom-tab')) {
