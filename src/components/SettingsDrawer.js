...
export function showSettingsDrawer(container, { onClose, actions = {} } = {}) {
  ...
  support.className = 'settings-support';

  const supportTitle = document.createElement('h2');
  supportTitle.className = 'settings-support-title';
  supportTitle.textContent = 'Apoie o MasterWhats';
  support.appendChild(supportTitle);

  const supportText = document.createElement('p');
  supportText.className = 'settings-support-text';
  supportText.textContent = 'Doe qualquer valor. A contribuição será usada para a infraestrutura e para manter o site no ar.';
  support.appendChild(supportText);

  const pixRow = document.createElement('div');
  pixRow.className = 'settings-support-row';

  const pixKey = document.createElement('code');
  pixKey.className = 'settings-support-key';
  pixKey.textContent = '84996961915';
  pixRow.appendChild(pixKey);

  const copyButton = document.createElement('button');
  copyButton.className = 'settings-support-copy';
  copyButton.type = 'button';
  copyButton.textContent = 'Copiar chave PIX';
  pixRow.appendChild(copyButton);
  support.appendChild(pixRow);

  const copyStatus = document.createElement('p');
  copyStatus.className = 'settings-support-status';
  copyStatus.setAttribute('aria-live', 'polite');
  support.appendChild(copyStatus);

  copyButton.addEventListener('click', async () => {
    const copied = await copyText(pixKey.textContent);
