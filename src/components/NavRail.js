...
const ICON_PAYMENTS = `<svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 10h18"/><path d="M7 15h3"/></svg>`;
const ICON_MEDIA = `<svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg>`;
...
export function renderNavRail(container, { avatarSrc, avatarName = 'Perfil', onSettings, onChat, onCalls, onPayments, onMedia } = {}) {
  ...
  rail.setActive = (which) => {
    for (const btn of rail.querySelectorAll('button[aria-label]')) {
      ...
      if (label === 'Pagamentos') btn.classList.toggle('active', which === 'payments');
      if (label === 'Mídias') btn.classList.toggle('active', which === 'media');
    }
  };
